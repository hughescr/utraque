...
//
// ClientModel is always the caller's (trimmed) spelling. An Anthropic-backed
// picker route carries no UpstreamModel: the Anthropic leg forwards
// Request.Raw byte-for-byte, so there is nothing to substitute, and
// Decision.UpstreamModel stays empty as that backend's contract requires.
