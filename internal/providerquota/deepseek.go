...
func NewDeepSeekClient(opts DeepSeekOptions) (*DeepSeekClient, error) {
	...
	if err != nil {
		return nil, quotaError(leg.DeepSeek, CodeConfiguration, false)
	}
	u.Path = strings.TrimSuffix(u.Path, "/") + "/user/balance"
	h, err := newHTTPSettings(leg.DeepSeek, u.String(), opts.HTTPClient, opts.Timeout, opts.MaxResponseBytes, opts.Now)
	if err != nil {
		return nil, err
	}
...
func (c *DeepSeekClient) Read(ctx context.Context) (Observation, error) {
	if c == nil {
		return Observation{}, quotaError(leg.DeepSeek, CodeConfiguration, false)
	}
	var payload deepSeekResponse
	if err := c.http.getJSON(ctx, leg.DeepSeek, c.cacheScope, map[string]string{
	...
	o := Observation{Source: leg.DeepSeek, CollectedAt: c.http.now().UTC(), Available: payload.Available, cacheScope: cacheScope}
