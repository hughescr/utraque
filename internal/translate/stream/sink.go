...
type Sink interface {
	...
	BlockStart(index int, block aschema.ContentBlock) error
	BlockDelta(index int, delta aschema.Delta) error
	BlockStop(index int) error
	MessageDelta(d MessageDelta) error
	MessageStop() error
	Error(err aschema.ErrorBody) error
	Ping() error
}
...
type wireMessage struct {
	...
	Content      []aschema.ContentBlock `json:"content"`
...
type wireBlockDelta struct {
	Type  string        `json:"type"`
	Index int           `json:"index"`
	Delta aschema.Delta `json:"delta"`
}
...
func (w *SSEWriter) MessageStart(m MessageStart) error {
	msg := wireMessageStart{
		Type: aschema.EventMessageStart,
		Message: wireMessage{
			ID:      m.ID,
			Type:    "message",
			Role:    aschema.RoleAssistant,
			Model:   m.Model,
			Content: []aschema.ContentBlock{},
			Usage:   aschema.Usage{InputTokens: m.InputTokens, OutputTokens: 0},
		},
	}
	return w.write(aschema.EventMessageStart, msg)
}
...
func (w *SSEWriter) BlockStart(index int, block aschema.ContentBlock) error {
	var cb any
	switch block.Type {
	case aschema.BlockText:
		cb = wireStartText{Type: aschema.BlockText, Text: block.Text}
	case aschema.BlockThinking:
		cb = wireStartThinking{Type: aschema.BlockThinking, Thinking: block.Thinking}
	case aschema.BlockToolUse:
		input := block.Input
		if len(input) == 0 {
			input = json.RawMessage(`{}`)
		}
		cb = wireStartToolUse{Type: aschema.BlockToolUse, ID: block.ID, Name: block.Name, Input: input}
	default:
		cb = block
	}
	return w.write(aschema.EventContentBlockStart, wireBlockStart{
		Type: aschema.EventContentBlockStart, Index: index, ContentBlock: cb,
	})
}

// BlockDelta writes content_block_delta.
func (w *SSEWriter) BlockDelta(index int, delta aschema.Delta) error {
	return w.write(aschema.EventContentBlockDelta, wireBlockDelta{
		Type: aschema.EventContentBlockDelta, Index: index, Delta: delta,
	})
}

// BlockStop writes content_block_stop.
func (w *SSEWriter) BlockStop(index int) error {
	return w.write(aschema.EventContentBlockStop, wireBlockStop{
		Type: aschema.EventContentBlockStop, Index: index,
	})
}
...
func (w *SSEWriter) MessageDelta(d MessageDelta) error {
	return w.write(aschema.EventMessageDelta, wireMessageDelta{
		Type:  aschema.EventMessageDelta,
...
func (w *SSEWriter) MessageStop() error {
	return w.write(aschema.EventMessageStop, map[string]string{"type": aschema.EventMessageStop})
}

// Error writes a mid-stream error frame.
func (w *SSEWriter) Error(errBody aschema.ErrorBody) error {
	return w.write(aschema.EventError, aschema.ErrorEvent{Type: aschema.EventError, Error: errBody})
}

// Ping writes a ping keepalive frame.
func (w *SSEWriter) Ping() error {
	return w.write(aschema.EventPing, map[string]string{"type": aschema.EventPing})
}
